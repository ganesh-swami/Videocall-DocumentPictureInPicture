...
const ActiveRoom = ({ roomName, userChoices, onLeave }: ActiveRoomProps) => {
  ...
  //@ts-ignore
  const handlePicInPic2 = (event) => {
    setReRender(true);
    //@ts-ignore
    event.view.document.querySelector('#handlepicinpic').style.display = 'none';
      ...
      {liveKitUrl && (
        ...
        >
          ...
          <button
              id="handlepicinpic"
              className="hidden"
              onClick={handlePicInPic2}
            >
              <div className="moon w-16 h-16 rounded-full flex items-center justify-center">
                <i
                  className={`icomoon-picInpic primaryColor dark:text-darkText text-[24px] lg:text-[32px]`}
                />{' '}
                Open PIP
              </div>
            </button>
